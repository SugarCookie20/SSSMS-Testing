        ...
        <div className="flex h-screen bg-gray-50 font-sans overflow-hidden">
            ...
            `}>
                ...
                <Link to="/faculty/profile" className="block px-6 py-6 border-b border-gray-200 hover:bg-gray-50 transition-colors group cursor-pointer">
                    <div className="flex items-center">
                        <div className="w-10 h-10 rounded-full bg-purple-100 flex items-center justify-center text-purple-600 group-hover:bg-purple-200 transition-colors overflow-hidden shrink-0">
                            {user?.profilePhoto ? (
                                <img src={getProfilePhotoUrl(user.profilePhoto)} alt="Profile" className="w-full h-full object-cover" />
                            ) : (
                ...
                <nav className="flex-1 overflow-y-auto py-4 px-3 space-y-1">
                    {navItems.map((item) => (
                        <Link
                            ...
                            to={item.href}
                            onClick={() => setSidebarOpen(false)}
                            className={`flex items-center px-3 py-2.5 text-sm font-medium rounded-lg transition-colors ${isActive(item.href) ? 'bg-purple-50 text-purple-700' : 'text-gray-600 hover:bg-gray-50'
                                }`}
                        >

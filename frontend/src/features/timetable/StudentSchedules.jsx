...
const StudentSchedules = () => {
            ...
            try {
                ...
                setSchedules({
                    exam: examRes.data.exists
                        ? { url: `${BASE_URL}/exams/view/${examRes.data.fileName}`, fileName: examRes.data.fileName }
                        : null,
                    timetable: timetableRes.data.exists
                        ? { url: `${BASE_URL}/timetable/view/${timetableRes.data.fileName}`, fileName: timetableRes.data.fileName }
                        : null,
                    calendar: schedulesRes.data.collegeCalendar?.exists
                        ? { url: `${BASE_URL}/schedules/view/${schedulesRes.data.collegeCalendar.fileName}`, fileName: schedulesRes.data.collegeCalendar.fileName }
                        : null,
                    academic: schedulesRes.data.academicSchedule?.exists
                        ? { url: `${BASE_URL}/schedules/view/${schedulesRes.data.academicSchedule.fileName}`, fileName: schedulesRes.data.academicSchedule.fileName }
                        : null,
                });
            } catch {
    ...
    const fileInfo = schedules[activeTab]; // { url, fileName } | null
                ...
                {TABS.map((tab) => {
                    ...
                    return (
                        <button
                            ...
                            className={`flex items-center px-4 py-2.5 rounded-lg border-2 text-sm font-semibold transition-all ${isActive
                                    ? colors.active
            ...
            <div className="flex-1 min-h-0">
                {fileInfo ? (
                    <div className="w-full h-full bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm">
                        <FileViewer url={fileInfo.url} fileName={fileInfo.fileName} title={activeTabData.label} className="h-full" />
                    </div>
                ) : (

...
import {
    ...
    CheckCircle, XCircle, Trash2, Eye, ArrowLeft
} from 'lucide-react';
...
const FacultySchedules = () => {
    ...
    const [myTimetableInfo, setMyTimetableInfo] = useState(null); // { url, fileName }
    ...
    const fetchData = async () => {
        try {
            ...
            if (timetableRes.data.exists) {
                setMyTimetableInfo({
                    url: `${BASE_URL}/timetable/view/${timetableRes.data.fileName}`,
                    fileName: timetableRes.data.fileName,
                });
            }
    ...
    return (
        <div className="max-w-5xl mx-auto">
            ...
            <div className="flex justify-between items-center mb-2">
                ...
                {activeTab === 'myTimetable' && myTimetableInfo && (
                    <a href={myTimetableInfo.url} download className="bg-blue-50 text-blue-600 px-4 py-2 rounded-lg flex items-center hover:bg-blue-100 text-sm font-medium">
                ...
                {TABS.map((tab) => {
                    ...
                    return (
                        <button
                            ...
                            className={`flex items-center px-4 py-2.5 rounded-lg border-2 text-sm font-semibold transition-all ${isActive ? colors.active : 'border-gray-200 text-gray-500 hover:border-gray-300 hover:bg-gray-50'
                                }`}
                        >
                ...
                <div className="h-[calc(100vh-320px)]">
                    {myTimetableInfo ? (
                        <div className="w-full h-full bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm">
                            <FileViewer url={myTimetableInfo.url} fileName={myTimetableInfo.fileName} title="My Timetable" className="h-full" />
                        </div>
                    ) : (
            ...
            {activeTab !== 'myTimetable' && (
                <>
                    <div className="bg-white p-8 rounded-xl border border-gray-200 shadow-sm mb-8">
                        {/* Status Banner */}
                        {status && (
                            <div className={`p-4 mb-6 rounded-lg flex items-center border ${status.type === 'success'
                                                    ...
                                                    <td className="px-6 py-4 text-center">
                                                        {hasFile && (
                                                            <div className="flex items-center justify-center gap-2">
                                                                <a
                                                                    href={`${BASE_URL}/schedules/view/${fileName}`}
                                                                    target="_blank"
                                                                    rel="noreferrer"

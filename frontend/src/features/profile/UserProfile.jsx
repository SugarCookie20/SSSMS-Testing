...
const UserProfile = () => {
    ...
    const handlePhotoUpload = async (e) => {
        ...
        if (!file || !isFaculty) return;

        const formData = new FormData();
        formData.append('file', file);

        try {
            await api.post('/faculty/me/photo', formData, {
        ...
        <div className="max-w-5xl mx-auto">
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-8 mb-6 flex items-center gap-6 shadow-sm">
                <div className="relative group">
                    <div className={`w-24 h-24 rounded-full flex items-center justify-center shrink-0 border-4 border-white shadow-sm overflow-hidden ${bgClass}`}>
                    ...
                    {isFaculty && (
                        <label className="absolute bottom-0 right-0 p-1.5 bg-purple-600 text-white rounded-full cursor-pointer hover:bg-purple-700 shadow-sm opacity-0 group-hover:opacity-100 transition-opacity">
                            <ArrowLeft className="w-4 h-4 hidden" />
                            <FileText className="w-3.5 h-3.5 hidden" />
                            <span className="text-[10px] uppercase font-bold leading-none tracking-wider px-1">Edit</span>
                        ...
                        {profile.professionalDevelopment.slice(0, 5).map((pd, i) => (
                            <div key={i} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg border border-gray-100">
                                <div className="flex items-center min-w-0">
                                    <span className={`text-xs font-medium px-2 py-0.5 rounded-full mr-3 shrink-0 ${pd.type === 'WORKSHOP' ? 'bg-blue-100 text-blue-700' :
            ...
            <div className="mt-6 bg-white rounded-xl border border-gray-200 p-8 shadow-sm">
                ...
                {passwordStatus.message && (
                    <div className={`p-3 mb-4 rounded-lg text-sm ${passwordStatus.type === 'success' ? 'bg-green-50 text-green-700 border border-green-200' : 'bg-red-50 text-red-700 border border-red-200'
                        }`}>
                        {passwordStatus.message}
                    </div>
                )}
                <form onSubmit={handleChangePassword} className="space-y-4 max-w-md">
                    ...
                    <button
                        ...
                        className={`flex items-center px-5 py-2.5 rounded-lg text-white font-medium shadow-sm transition-colors disabled:opacity-50 ${isFaculty ? 'bg-purple-600 hover:bg-purple-700' : 'bg-blue-600 hover:bg-blue-700'
                            }`}
                    >

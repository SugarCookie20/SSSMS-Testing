...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (result.success) {
      ...
      if (!userRole.includes(selectedRole.toLowerCase())) {
        setError(`Invalid credentials for ${selectedRole.charAt(0).toUpperCase() + selectedRole.slice(1)} role.`);
        await logout(); // Ensure they are logged out if role mismatches
        setLoading(false);
        return;
      }
        ...
        <div className="w-full max-w-md">
          ...
          <div className="flex mb-6 bg-gray-100 rounded-lg p-1">
            <button
              ...
              className={`flex-1 flex items-center justify-center py-2 px-4 rounded-md transition-all ${selectedRole === "student"
            ...
            <button
              ...
              className={`flex-1 flex items-center justify-center py-2 px-4 rounded-md transition-all ${selectedRole === "faculty"
                  ? "bg-blue-600 text-white shadow-sm"
                  : "text-gray-600 hover:text-gray-900"
                }`}
            >
            ...
            <button
              ...
              className={`flex-1 flex items-center justify-center py-2 px-4 rounded-md transition-all ${selectedRole === "admin"
                  ? "bg-blue-600 text-white shadow-sm"
                  : "text-gray-600 hover:text-gray-900"
                }`}
            >
